...
class SeatingPlan extends React.Component {
  ...
  cellPress = (cell) => {
    if (this.state.selectedIndex == 1 && cell.id % 15 != 0 && !cell.isStored) {
      ...
      if (cell.id % 15 !== 0 && (cell.id + 1) % 15 != 0 && cell.id < 405) {
        // magic numbers here
        const cell1 = this.state.grid[cell.id];
        const cell2 = this.state.grid[cell.id + 1];
      ...
      if (cell.id % 15 !== 0 && (cell.id + 1) % 15 != 0 && cell.id < 390) {
        // magic numbers here
        const cell1 = this.state.grid[cell.id];
        const cell2 = this.state.grid[cell.id + 1];
        const cell3 = this.state.grid[cell.id + 2];
  ...
  bookingPressed = () => {
    setTimeout(() => {
      Alert.alert("Please Unbook your seat");
    }, 5 * 1000);
    this.setState({
      grid: this.props.seatingPlanGrid,
    });
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    this.state.selectedIndex == 5
                      ? this.cellPressErase(this.state.grid[index])
                      : this.cellPress(this.state.grid[index]);
                  }}
                />
              ))
            : Object.keys(this.props.seatingPlanGrid).map((index) => (
                <TouchableOpacity
                  key={index}
                  style={seatStyle(this.props.seatingPlanGrid[index])}
                  onPress={() => {
                    this.state.selectedIndex == 5
                      ? this.cellPressErase(this.props.seatingPlanGrid[index])
                      : this.cellPress(this.props.seatingPlanGrid[index]);
                  }}
                />
              ))}
        </View>
        {this.props.user.userType == FOODCENTRE_USER && this.state.isEditing ? (
          <View style={styles.settingsButtons}>
            ...
            <TouchableOpacity onPress={this.eraseAll} style={styles.seatButton}>
              <Text>Erase All</Text>
            </TouchableOpacity>
            <TouchableOpacity
